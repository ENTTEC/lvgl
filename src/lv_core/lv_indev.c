...
static void indev_keypad2_proc(lv_indev_t * i, lv_indev_data_t * data);
...
void _lv_indev_read_task(lv_task_t * task)
{
    ...
    do {
        ...
        if(indev_act->driver.type == LV_INDEV_TYPE_POINTER) {
            ...
            indev_keypad2_proc(indev_act, &data);
        }
...
static void indev_keypad2_proc(lv_indev_t * i, lv_indev_data_t * data)
{
    ...
    if(data->state == LV_INDEV_STATE_PR && prev_state == LV_INDEV_STATE_REL) {
        ...
        if(data->key == LV_KEY_ENTER) {
            bool editable = false;
            lv_obj_type_t tmp;
            lv_obj_get_type(indev_obj_act, &tmp);

            indev_obj_act->signal_cb(indev_obj_act, LV_SIGNAL_GET_EDITABLE, &editable);
            if (strcmp(tmp.type[0],"lv_spinbox")!=0 &&
                    strcmp(tmp.type[0],"lv_dropdown")!=0)
            {
                editable=false;
            }
            ...
            } else if (editable && lv_group_get_editing(g) == true)
            {
                lv_group_set_editing(g, false); /*Enter edit mode on repeated Enter press*/
            }
            else {
                /*Send the ENTER as a normal KEY*/
                lv_group_send_data(g, LV_KEY_ENTER);
